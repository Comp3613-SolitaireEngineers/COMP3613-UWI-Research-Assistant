...
class AuthorPublication(db.Model):
    ...
    author_id = db.Column(db.Integer, db.ForeignKey('author.author.id')) 
    publication_id = db.Column(db.Integer, db.ForeignKey('publication.publication.id'))  

    def __init__(self,author_id,publication_id): self.author_id= author_id, self.publication_id = publication_id
